...
def add_one_url_info():
    ...
    if randint(0,1):
        with conn, conn.cursor() as cur:
            cur.execute("""SELECT registered_urls.url
        ...
        with conn, conn.cursor() as cur:
            cur.execute("""SELECT possible_urls.url
    ...
    if url != -1:
        ...
        with conn, conn.cursor() as cur:
            url_data = str(url[1])
            cur.execute(sql_code,(company_url,events['register_date'],events['expire_date'],events['last_update_date'],url_data))
            conn.commit()
